...
load_dotenv()
# --- Configuration ---
ASSEMBLYAI_API_KEY = os.getenv("ASSEMBLYAI_API_KEY")  # Replace with your actual API key
...
def record_fixed_duration(seconds=10):
    """Record audio for a fixed duration (10–20 sec) and save as WAV."""
    ...
    total_chunks = int(SAMPLE_RATE / FRAMES_PER_BUFFER * seconds)
    ...
    # Save file
    filename = f"fixed_recording_{int(time.time())}.wav"
...
def run_fixed_duration(seconds=10):
    """Run streaming STT for fixed duration (10–20 sec)"""
    ...
    try:
        ...
        print(f"Recording + STT for {seconds} seconds...")
    except Exception as e:
        print(f"Mic error: {e}")
        return
